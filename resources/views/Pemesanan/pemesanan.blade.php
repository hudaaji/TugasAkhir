                        ...
                        @else
                            @if ($p -> keterangan == 0)
                                ...
                                <td>
                                    ...
                                    <script>
                                    this["countDownDate"+{{$modals}}] = new Date("{{$p->waktu}}").getTime();

                                    // Update the count down every 1 second
                                    this["x"+{{$modals}}] = setInterval(function() {

                                    // Get today's date and time
                                    this["now"+{{$modals}}] = new Date().getTime();

                                    // Find the distance between now and the count down date
                                    this["distance"+{{$modals}}] = this["countDownDate"+{{$modals}}] - this["now"+{{$modals}}];

                                    // Time calculations for days, hours, minutes and seconds
                                    this["days"+{{$modals}}] = Math.floor(this["distance"+{{$modals}}] / (1000 * 60 * 60 * 24));
                                    this["hours"+{{$modals}}] = Math.floor((this["distance"+{{$modals}}] % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
                                    this["minutes"+{{$modals}}] = Math.floor((this["distance"+{{$modals}}] % (1000 * 60 * 60)) / (1000 * 60));
                                    this["seconds"+{{$modals}}] = Math.floor((this["distance"+{{$modals}}] % (1000 * 60)) / 1000);

                                    // Display the result in the element with id="demo"
                                    document.getElementById("demo-{{$modals}}").innerHTML = this["days"+{{$modals}}] + "d " + this["hours"+{{$modals}}] + "h "
                                    + this["minutes"+{{$modals}}] + "m " + this["seconds"+{{$modals}}] + "s ";

                                    // If the count down is finished, write some text
                                    if (this["distance"+{{$modals}}] < 0) {
                                        clearInterval(this["x"+{{$modals}}]);
                                        document.getElementById("demo-{{$modals}}").innerHTML = "EXPIRED";
                                    }
                                    }, 1000);
                                    </script>
                                </td>
